...
export const createPppoeClient = async (
  ...
  try {
    ...
    // Check duplicate
    const existing = await conn.write('/ppp/secret/print', [`?name=${data.name}`]);
    if (existing.length) {
      await conn.close();
...
export const disableClient = async (
  ...
  try {
    ...
    const sessions = await conn.write('/ppp/active/print', [`?name=${pppoeUsername}`]);
...
export const listPppoeClients = async (): Promise<{ success: boolean; data?: PppoeSecret[]; error?: string }> => {
  ...
  try {
    ...
    const raw = await conn.write('/ppp/secret/print');
    await conn.close();
...
export const listActiveSessions = async (): Promise<{ success: boolean; data?: ActiveSession[]; error?: string }> => {
  ...
  try {
    ...
    const raw = await conn.write('/ppp/active/print');
    await conn.close();
